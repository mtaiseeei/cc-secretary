...
ユーザーに話しかける前に、必ず `plugins/cc-secretary/rules/plain-language.md`（やさしい言葉ルール）を読むこと。
...
**生成手順**（`templates/` の雛形を実体化して穴埋めする。同じ回答なら毎回同じ構造になる）:

1. 作業中のフォルダに `secretary/` を作る。
2. `templates/` の中身を `secretary/` にコピーする（雛形の実体化）。
   - `templates/AGENTS.md` → `secretary/AGENTS.md`
   - `templates/CLAUDE.md` → `secretary/CLAUDE.md`
   - `templates/inbox/`・`templates/docs/`・`templates/projects/`（`.gitkeep` ごと） → `secretary/` 配下へ
   - `templates/memory/MEMORY.md` → `secretary/memory/MEMORY.md`
   - `templates/memory/preferences.md` → `secretary/memory/preferences.md`
   - `templates/memory/decisions/_first-decision.md` → `secretary/memory/decisions/{{CREATED_DATE}}-decisions.md`（名前を日付にする）
...
- 生成物の構造・規律の正本: プロジェクトの `docs/spec/domain.md` / `docs/spec/constraints.md`
- 雛形: リポジトリ直下の `templates/`
