...
function renderOAuthFailure(message = state.oauth?.message) {
  progress(0);
  app.innerHTML = `<p class="eyebrow">接続を確認できません</p><h1>Google認証を完了できませんでした。</h1><p class="lead error" role="alert">${escape(message)}</p><p class="notice">拒否、Audience不一致、管理者ブロック、API無効、<code>redirect_uri_mismatch</code> は対応が異なります。表示された理由を管理者へ伝えてください。</p>${actions("最初から確認する", "変更せず終了")}`;
