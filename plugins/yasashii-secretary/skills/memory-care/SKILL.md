...
- 決まったこと（意思決定）は、その場で節目プロトコルの厳密な1行確認を行い、**次の別ターンで明示的な了承を得た後だけ** `remember-decision` に渡す。確認なしで渡さない。
- `remember-decision` は過去の決定を直さず、新しい日付ファイルへ追加する。変更時は `変更: 「旧決定」(旧日付) → 「新決定」（理由）` の形で新しい行を足す。
- 結論が出ていない相談でも、背景として価値がある場合は「要点を案件メモに残しますね」と1行で確認し、了承後だけ `topic-add` に渡す。会話全文や外部データ本文は保存しない。
...
4. 確認行の前後に、挨拶、解釈、補足、曖昧さの質問、「よろしければ」等の再確認、空行、2行目を足さない。この1行自体が記録可否の確認である。
...
6. 会話の締めで、当日の`--type decisions --from <今日> --to <今日>`が0件なら会話を読み返す。
   決定候補があれば1行で確認し、無ければ「今日は新しい決定はありませんでした」と伝える。確認は1回だけにする。

確認ターンの出力例（各出力は表示した1行だけ）:
...
`preferences.md` の「決定の確認」が「都度」なら上記の厳密1行確認を行う。「まとめて」なら候補を未確認のまま記録せず、
会話の締めで候補を列挙して一括確認する。了承後に各候補を記録し、decidedが0件の日の拾い漏れ確認はどちらでも省略しない。

### 結論のない相談は要点だけ確認する

相談が一区切りし、背景・経緯・固有名詞が今後も役立つ場合は、
`要点を案件メモに残しますね: <1行の要点>`と確認する。了承後だけ`topic-add`を実行する。
