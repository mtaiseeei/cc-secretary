...
> まず5つだけ伺います。そのあと秘書ディレクトリ（secretary/）を用意し、最初のコミットを記録します（数十秒で終わります）。
...
- `AGENTS.md`、`MEMORY.md`等を用意し、最初のlocal commitを作成したこと。pushしていないこと。
- 次に試せる操作と、「設定はいつでも『設定変えたい』で変更できます」という案内。
