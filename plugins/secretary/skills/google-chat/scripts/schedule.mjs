...
export function renderGoogleChatWorkflow(interval, enabled, identity = {}) {
  ...
  return [
    ...
    "          fetch-depth: 0",
    "      - name: 通常スペース候補を確認",
    "        id: google_chat_discovery",
    "        if: github.event_name == 'workflow_dispatch' && inputs.mode == 'discover'",
    "        continue-on-error: true",
    "        env:",
    "          GOOGLE_OAUTH_CLIENT_ID: ${{ secrets.GOOGLE_OAUTH_CLIENT_ID }}",
    "          GOOGLE_OAUTH_CLIENT_SECRET: ${{ secrets.GOOGLE_OAUTH_CLIENT_SECRET }}",
    "          GOOGLE_OAUTH_REFRESH_TOKEN_GCHAT: ${{ secrets.GOOGLE_OAUTH_REFRESH_TOKEN_GCHAT }}",
    "          GOOGLE_CHAT_DISCOVERY_CORRELATION_ID: ${{ inputs.correlation_id }}",
    "        run: node google-chat/scripts/discovery.mjs \"$GITHUB_WORKSPACE\"",
    "      - name: Google Chatから取得",
    "        id: google_chat",
    "        if: github.event_name == 'schedule' || inputs.mode != 'discover'",
    "        continue-on-error: true",
    "        env:",
    "          GOOGLE_OAUTH_CLIENT_ID: ${{ secrets.GOOGLE_OAUTH_CLIENT_ID }}",
    "          GOOGLE_OAUTH_CLIENT_SECRET: ${{ secrets.GOOGLE_OAUTH_CLIENT_SECRET }}",
    ...
    "      - name: 取得結果を同じrepoへ記録",
    "        if: always() && (steps.google_chat.outcome != 'skipped' || steps.google_chat_discovery.outcome != 'skipped')",
    "        run: |",
    "          git config user.name \"" + botName + "\"",
    "          git config user.email \"" + botEmail + "\"",
    "          if [ \"${{ inputs.mode }}\" = \"discover\" ]; then",
    "            git add google-chat/spaces-discovery.json",
    "          else",
    ...
    "          fi",
    "      - name: 取得失敗を通知",
    "        if: (steps.google_chat.outcome != 'skipped' && steps.google_chat.outcome != 'success') || (steps.google_chat_discovery.outcome != 'skipped' && steps.google_chat_discovery.outcome != 'success')",
    "        run: |",
