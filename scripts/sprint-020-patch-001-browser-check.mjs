...
async function observe() {
  return evaluate(`(()=>{const app=document.querySelector('#app');const primary=[app.querySelector('h1')?.textContent,...[...app.querySelectorAll('[data-copy-role="now"],[data-copy-role="result"],[data-copy-role="status"],[data-copy-role="error"],.actions .button')].map(x=>x.textContent)].join(' ');return {screen:app.dataset.screen,state:app.dataset.state,service:app.getAttribute('aria-label'),heading:app.querySelector('h1')?.textContent,primary,forbidden:/\\b(?:wizard|workflow|commit|push|Repository Secret|loopback|runtime|scope|token|OAuth client JSON|Sprint[- ]?\\d*)\\b/i.test(primary),details:[...app.querySelectorAll('details')].map(x=>({open:x.open,summary:x.querySelector('summary')?.textContent})),safety:[...app.querySelectorAll('[data-copy-role="safety"] strong')].map(x=>x.textContent),cta:[...app.querySelectorAll('.actions .button')].map(x=>({name:x.getAttribute('aria-label')||x.textContent,bg:getComputedStyle(x).backgroundColor,fg:getComputedStyle(x).color,height:x.getBoundingClientRect().height})),overflow:document.documentElement.scrollWidth>innerWidth,width:innerWidth,actions:app.querySelector('.actions')?getComputedStyle(app.querySelector('.actions')).flexDirection:null}})()`);
}
...
const report = { chatwork: [], google: [], responsive: [] };
...
report.chatwork.push(await screen("chatwork-select-rooms"));
...
await evaluate(`fetch('/api/oauth/client',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({clientJson:JSON.stringify({installed:{client_id:'fixture-id-2',client_secret:'fixture-secret-2',auth_uri:'https://accounts.google.com/o/oauth2/v2/auth',token_uri:'https://oauth2.googleapis.com/token',redirect_uris:['http://localhost']}})})}).then(r=>r.json()).then(()=>fetch('/api/oauth/synthetic',{method:'POST',headers:{'content-type':'application/json'},body:'{"mode":"success"}'})).then(r=>r.json())`);
await send("Page.reload", { ignoreCache: true }); report.google.push(await screen("google-chat-select-spaces"));
...
await open(chatworkUrl, 390, 844); await screen("chatwork-select-rooms"); const chatMobile = await observe(); report.responsive.push({ service: "chatwork", mode: "mobile", ...chatMobile }); await screenshot("chatwork-mobile.png");
await open(chatworkUrl, 720, 450, 2); await screen("chatwork-select-rooms"); const chatZoom = await observe(); report.responsive.push({ service: "chatwork", mode: "200%", ...chatZoom }); await screenshot("chatwork-zoom200.png");
await open(googleSettingsUrl, 390, 844); await screen("google-chat-settings-select-spaces"); const googleMobile = await observe(); report.responsive.push({ service: "google-chat", mode: "mobile", ...googleMobile }); await screenshot("google-chat-mobile.png");
await open(googleSettingsUrl, 720, 450, 2); await screen("google-chat-settings-select-spaces"); const googleZoom = await observe(); report.responsive.push({ service: "google-chat", mode: "200%", ...googleZoom }); await screenshot("google-chat-zoom200.png");
...
const passed = browserErrors.length === 0 && all.every((item) => !item.forbidden && !item.overflow && item.details.every((detail) => !detail.open) && item.cta.every((button) => button.height >= 44) && serviceColor(item))
  ...
  && report.google.some((item) => item.screen === "google-chat-authorize-failure") && report.google.some((item) => item.screen === "google-chat-initial-result-empty") && report.google.some((item) => item.screen === "google-chat-review" && item.safety.length === 5)
