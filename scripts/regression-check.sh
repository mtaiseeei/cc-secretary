...
MORNING_SKILL="$PLUGIN/skills/morning/SKILL.md"
EVENING_SKILL="$PLUGIN/skills/evening/SKILL.md"
...
check "morning/SKILL.md が存在" "[ -f '$MORNING_SKILL' ]"
check "evening/SKILL.md が存在" "[ -f '$EVENING_SKILL' ]"
...
MONAME="$(name_of "$MORNING_SKILL")"
EVNAME="$(name_of "$EVENING_SKILL")"
...
check "morning の name が 'morning'" "[ '$MONAME' = 'morning' ]"
check "evening の name が 'evening'" "[ '$EVNAME' = 'evening' ]"
check "name が一意（重複なし）" \
  "[ \"\$(printf '%s\n' '$SNAME' '$ONAME' '$MNAME' '$GNAME' '$DNAME' '$MSNAME' '$NNAME' '$CNAME' '$BNAME' '$MONAME' '$EVNAME' | sort -u | wc -l | tr -d ' ')\" = '11' ]"
...
# 雛形をコピー（.gitkeep 含む）
cp -R "$TEMPLATES/." "$DEST/"
# onboarding の手順どおり、templates/prompts/（定時実行の貼り付け用テンプレート）は秘書ディレクトリにコピーしない
rm -rf "$DEST/prompts"
# 決定ログの雛形を日付名にリネーム
...
mk_sec(){ # $1=dest
  cp -R "$TEMPLATES/." "$1/"
  rm -rf "$1/prompts"   # onboarding と同じく prompts/ は秘書ディレクトリに入れない
...
# 記録 HEAD の更新履歴: sprint-005 時点は 56ce6938…。sprint-007 着手時、複製元がユーザー自身の
# コミット（PR #1/#2 マージ・LICENSE 修正）で fb9c3037… に進んでいたため、作業ツリーがクリーンである
# ことを確認のうえベースラインを取り直した（cc-secretary 側からの書き込みではない。docs/progress/sprint-007.md 参照）。
check "A1: 複製元 git HEAD が記録値と一致" "[ \"\$(git -C '$HARNESS_SRC' rev-parse HEAD 2>/dev/null)\" = 'fb9c30375dac5d4458ed0f522b3469cff2f6b949' ]"
...
# ---------------------------------------------------------------------------
section "14. 朝夕の定例化（morning / evening・sprint-007）"
# ---------------------------------------------------------------------------
TPL_MORNING="$PLUGIN/templates/prompts/morning-brief.md"

# --- 受入1: 定時実行テンプレートの実在と morning からの ${CLAUDE_PLUGIN_ROOT} 相対参照 ---
check "定時実行テンプレートが templates/prompts/ に実在" "[ -f '$TPL_MORNING' ]"
check "morning がテンプレートを \${CLAUDE_PLUGIN_ROOT} 相対で参照" \
  "grep -q '\${CLAUDE_PLUGIN_ROOT}/templates/prompts/morning-brief.md' '$MORNING_SKILL'"

# --- 受入2: テンプレートの自己完結（開発専用ファイル・環境変数に依存しない）---
check "テンプレートが docs/spec・docs/sprints を参照しない" "! grep -qE 'docs/spec|docs/sprints' '$TPL_MORNING'"
check "テンプレートの文面が環境変数に依存しない（CLAUDE_PLUGIN_ROOT 不使用）" "! grep -q 'CLAUDE_PLUGIN_ROOT' '$TPL_MORNING'"
check "テンプレートが朝ブリーフ起動として意図が読める（/secretary＋ブリーフ）" \
  "grep -q '/secretary' '$TPL_MORNING' && grep -q 'ブリーフ' '$TPL_MORNING'"
check "テンプレートが根拠ルールを指示（サービス名＋リンク/ID＋日付）" "grep -q 'サービス名' '$TPL_MORNING'"
check "テンプレートが本文非保存・不可逆操作禁止を指示" \
  "grep -q '保存しない' '$TPL_MORNING' && grep -q '不可逆' '$TPL_MORNING'"
check "テンプレートに置換用の穴（{{ }}）が無い（そのまま貼れる）" "! grep -q '{{' '$TPL_MORNING'"

# --- 受入3: セットアップ案内（3方式・未設定でも成立・対訳・しおり連携）---
check "morning が scheduled tasks を本命として案内" "grep -q 'scheduled tasks' '$MORNING_SKILL' && grep -q '本命' '$MORNING_SKILL'"
check "morning が Routines を代替として案内" "grep -q 'Routines' '$MORNING_SKILL' && grep -q '代替' '$MORNING_SKILL'"
check "morning が手動フォールバック（おはよう）を案内" "grep -q 'おはよう' '$MORNING_SKILL'"
check "morning が『未設定でも同じブリーフが得られる』を明示" \
  "grep -q '設定していなくても' '$MORNING_SKILL' || grep -q '未設定' '$MORNING_SKILL'"
check "morning の英語 UI 項目に対訳がある（Scheduled tasks／スケジュール済みタスク等）" \
  "grep -q 'スケジュール済みタスク' '$MORNING_SKILL' && grep -q '設定（Settings）' '$MORNING_SKILL'"
check "morning が Routines の制約（プラグイン/ローカルへ届かない場合）を正直に案内" \
  "grep -q '届かない' '$MORNING_SKILL'"
check "morning が設定前にしおりを書く（resume-write）" "grep -q 'resume-write' '$MORNING_SKILL'"
check "morning が設定後にしおりを閉じる（resume-clear）" "grep -q 'resume-clear' '$MORNING_SKILL'"
check "morning が UI 名称変更に耐える型（設定→定時実行の登録→貼り付け）を持つ" \
  "grep -q '貼り付け' '$MORNING_SKILL' && grep -q '設定' '$MORNING_SKILL'"

# --- 受入4: 朝ブリーフの振る舞い（daily を土台・根拠・非保存・未接続フォールバック・拡張可能）---
check "morning が daily を段階ロード（二重実装しない）" "grep -q 'skills/daily/SKILL.md' '$MORNING_SKILL'"
check "morning に根拠ルール（サービス名＋リンク/ID＋日付）" "grep -q 'サービス名' '$MORNING_SKILL' && grep -q '日付' '$MORNING_SKILL'"
check "morning が外部本文をローカルに保存しない旨" "grep -q '保存しない' '$MORNING_SKILL'"
check "morning が未接続でも壊れず接続案内へ落ちる" "grep -q '未接続でも壊さない' '$MORNING_SKILL' && grep -q 'setup-google/SKILL.md' '$MORNING_SKILL'"
check "morning のブリーフが項目を後から追加できるブロック構成" "grep -q 'ブロック' '$MORNING_SKILL' && grep -q '差し込' '$MORNING_SKILL'"
check "morning が推測で予定を作らない旨" "grep -q '推測' '$MORNING_SKILL'"
# 朝ブリーフのローカル書き込みは TODO＋根拠参照まで（サンプル TODO のドライランは下の evening 検証と共用）

# --- 受入5: 夕方の消し込み・繰越（確認保護・決定的シーム・封じ込め）---
# ドライラン用の秘書ディレクトリを作り、サンプル TODO を3件入れる
S14="$WORK/s14/secretary"; mkdir -p "$WORK/s14"; mk_sec "$S14"
bash "$WT" todo-add "$S14" "請求書を送る" "Gmail | https://mail.google.com/a | 2026-07-15" >/dev/null 2>&1
bash "$WT" todo-add "$S14" "打合せ資料の確認" "Googleカレンダー | evt-123 | 2026-07-15" >/dev/null 2>&1
bash "$WT" todo-add "$S14" "資料への返信" "Gmail | https://mail.google.com/b | 2026-07-15" >/dev/null 2>&1
todo_count(){ grep -c '^- \[ \]' "$S14/inbox/todo.md"; }
check "ドライラン: サンプル TODO が3件入る" "[ \"\$(todo_count)\" = '3' ]"
# 朝ブリーフの前提: ローカルに残るのは TODO＋根拠参照だけ（本文キャッシュを作らない）
check "ドライラン: ローカルは TODO＋根拠参照のみ（キャッシュ層なし）" \
  "! find '$S14' -type d -name '10_sources' | grep -q . && grep -q '根拠: Gmail' '$S14/inbox/todo.md'"

# (a) 消し込みは未確認では実行されない（何を消すかを提示して中断）
bash "$WT" todo-done "$S14" 2 >/tmp/cc_done_warn.$$ 2>&1
DONE_RC=$?
check "todo-done: 未確認では消さない（exit 3）" "[ '$DONE_RC' -eq 3 ]"
check "todo-done: 未確認時も3件のまま残る" "[ \"\$(todo_count)\" = '3' ]"
check "todo-done: 警告に『何を消すか』が出る" "grep -q '打合せ資料の確認' /tmp/cc_done_warn.$$"
rm -f /tmp/cc_done_warn.$$
# (b) 確認つき消し込みで対象だけ消える
bash "$WT" todo-done "$S14" 2 --confirm >/dev/null 2>&1
check "todo-done: 確認つきで完了項目が消える" "[ $? -eq 0 ] && ! grep -q '打合せ資料の確認' '$S14/inbox/todo.md'"
check "todo-done: 他の項目は残る（2件）" "[ \"\$(todo_count)\" = '2' ] && grep -q '請求書を送る' '$S14/inbox/todo.md' && grep -q '資料への返信' '$S14/inbox/todo.md'"
# (c) 繰越は未確認では実行されない
bash "$WT" todo-carry "$S14" 2 2026-07-16 >/dev/null 2>&1
check "todo-carry: 未確認では変更しない（exit 3）" "[ $? -eq 3 ] && ! grep -q '繰越' '$S14/inbox/todo.md'"
# (d) 確認つき繰越で翌日分として残る（消えない）
bash "$WT" todo-carry "$S14" 2 2026-07-16 --confirm >/dev/null 2>&1
check "todo-carry: 確認つきで繰越が付く" "[ $? -eq 0 ] && grep -q '資料への返信.*（繰越: 2026-07-16）' '$S14/inbox/todo.md'"
check "todo-carry: 繰越項目は消えずに残る（2件のまま）" "[ \"\$(todo_count)\" = '2' ]"
# (e) 不正な指定は使い方エラーで拒否（偽装成功しない）
bash "$WT" todo-done "$S14" 99 --confirm >/dev/null 2>&1
check "todo-done: 範囲外の番号は拒否（exit≠0）" "[ $? -ne 0 ]"
bash "$WT" todo-done "$S14" abc --confirm >/dev/null 2>&1
check "todo-done: 数字でない番号は拒否（exit≠0）" "[ $? -ne 0 ]"
bash "$WT" todo-carry "$S14" 1 "16日" --confirm >/dev/null 2>&1
check "todo-carry: 不正な日付は拒否（exit≠0）" "[ $? -ne 0 ]"
# (f) 封じ込め: inbox が外向き symlink なら消し込み・繰越を拒否し、外部が不変
mkdir -p "$WORK/s14/evil"; printf -- '- [ ] 外部のTODO\n' > "$WORK/s14/evil/todo.md"
S14B="$WORK/s14/sec-b"; mkdir -p "$S14B"; mk_sec "$S14B"
rm -rf "$S14B/inbox"; ln -s "$WORK/s14/evil" "$S14B/inbox"
bash "$WT" todo-done "$S14B" 1 --confirm >/dev/null 2>&1
check "todo-done: inbox symlink 越えは拒否（exit 3）" "[ $? -eq 3 ]"
bash "$WT" todo-carry "$S14B" 1 2026-07-16 --confirm >/dev/null 2>&1
check "todo-carry: inbox symlink 越えは拒否（exit 3）" "[ $? -eq 3 ]"
check "封じ込め: 拒否後も外部の todo.md が不変" "[ \"\$(cat '$WORK/s14/evil/todo.md')\" = '- [ ] 外部のTODO' ]"

# --- 受入6: 夕方の節目コミット（日本語・push なし）---
S14_BEFORE="$(git -C "$S14" rev-list --count HEAD 2>/dev/null)"
bash "$TOOLS" commit "$S14" "一日の締め（TODO の消し込みと明日への繰越）" >/dev/null 2>&1
check "夕方の締めで節目コミットが増える" "[ \"\$(git -C '$S14' rev-list --count HEAD)\" -gt '$S14_BEFORE' ]"
S14MSG="$(git -C "$S14" log -1 --pretty=%s 2>/dev/null)"
check "締めのコミットが日本語メッセージ" "printf '%s' \"$S14MSG\" | LC_ALL=C grep -q '[^ -~]'"
check "締めのコミット後もリモート未設定（push なし）" "[ -z \"\$(git -C '$S14' remote 2>/dev/null)\" ]"

# --- evening スキルの設計（確認を挟む・シーム経由・記憶を量産しない・3行型）---
check "evening が消し込み・繰越に --confirm 手順を明記" "grep -q -- '--confirm' '$EVENING_SKILL'"
check "evening が『OK をもらってから』実行する設計" "grep -q 'OK をもらってから' '$EVENING_SKILL'"
check "evening が無確認の一括削除をしない旨" "grep -q '無確認の一括削除をしない' '$EVENING_SKILL'"
check "evening が決定的シーム（todo-done / todo-carry）を使う" "grep -q 'todo-done' '$EVENING_SKILL' && grep -q 'todo-carry' '$EVENING_SKILL'"
check "evening が記憶ファイルを毎日量産しない旨" "grep -q '量産しない' '$EVENING_SKILL'"
check "evening が節目コミット（push しない）" "grep -q 'memory-tools.sh commit' '$EVENING_SKILL' && grep -q 'push はしない' '$EVENING_SKILL'"
check "evening の締めが3行型（今日できたこと／明日へ送ること／次）" \
  "grep -q '今日できたこと' '$EVENING_SKILL' && grep -q '明日へ送ること' '$EVENING_SKILL' && grep -q '次に何が起きるか' '$EVENING_SKILL'"

# --- 受入7: 非エンジニア体験（語彙方針・plain-language 参照・3行型）---
check "morning が plain-language を参照" "grep -q 'plain-language.md' '$MORNING_SKILL'"
check "evening が plain-language を参照" "grep -q 'plain-language.md' '$EVENING_SKILL'"
check "sprint-007 の新規文言に家系メタファーが無い" \
  "! grep -qE '秘書の家|この家|お家|おうち' '$MORNING_SKILL' '$EVENING_SKILL' '$TPL_MORNING'"
check "morning の報告に『次』の一言がある" "grep -q '次は' '$MORNING_SKILL'"

# --- ルーターへの組み込み（語彙→段階ロード）---
check "ルーターに朝ブリーフ（おはよう→morning）の行がある" \
  "grep -q 'おはよう' '$SECRETARY_SKILL' && grep -q 'skills/morning/SKILL.md' '$SECRETARY_SKILL'"
check "ルーターに夕方の振り返り（おつかれ→evening）の行がある" \
  "grep -q 'おつかれ' '$SECRETARY_SKILL' && grep -q 'skills/evening/SKILL.md' '$SECRETARY_SKILL'"

# --- 受入8: 安全・規律（常駐・独自 cron・自動送信を作らない／構造の決定性）---
check "新規ファイルに常駐/独自スケジューラの実装が無い（crontab/launchctl/systemd 不使用）" \
  "! grep -qiE 'crontab|launchctl|launchd|systemd' '$MORNING_SKILL' '$EVENING_SKILL' '$TPL_MORNING' '$WT'"
check "morning が『常駐・独自 cron を作らない』を明記" "grep -q '常駐' '$MORNING_SKILL'"
check "テンプレート追加後も生成される秘書ディレクトリに prompts/ が混ざらない（決定性）" "[ ! -e '$DEST/prompts' ]"
check "onboarding が prompts/ をコピーしない旨を明記" "grep -q 'prompts' '$ONBOARD_SKILL'"
check "夕方ドライランの生成物に資格情報の実値が無い" \
  "! grep -rnEi '(password|api[_-]?key|secret|token)\\s*[:=]\\s*[A-Za-z0-9]' '$S14/inbox' '$S14/memory'"
