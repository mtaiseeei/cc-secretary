...
check_eval "wizardは初回設定と設定変更の結果を分離" "grep -q 'configuration-change' '$CHATWORK/scripts/wizard-server.mjs' && grep -q '設定変更が完了しました' '$CHATWORK/assets/wizard/app.js' && grep -q '現在の対象ルーム' '$CHATWORK/assets/wizard/app.js' && grep -q '自動実行' '$CHATWORK/assets/wizard/app.js'"
...
check_eval "6間隔・概算実行回数・料金details" "for text in '30分ごと' '1時間ごと（おすすめ）' '3時間ごと' '6時間ごと' '12時間ごと' '手動のみ' '料金と実行時間について' '2,000回の実行枠ではありません'; do grep -q \"\$text\" '$CHATWORK/assets/wizard/app.js' || exit 1; done"
