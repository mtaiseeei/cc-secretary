...
- 合計: **53状態**（Chatwork 23、Google Chat 30）
...
| `node scripts/sprint-020-patch-001-browser-check.mjs --cdp http://127.0.0.1:9231 --chatwork-url http://127.0.0.1:18784/ --google-new-url http://127.0.0.1:18783/ --google-settings-url http://127.0.0.1:18782/` | `BROWSER_PASS=30 BROWSER_FAIL=0 SCREENS=30` |
...
copy検査は、53状態のinventory、必須意味、heading／button／label／accessible name、DOMのscreen／state、primary禁止語を確認する。安全項目欠落、画面名変更、primary禁止語混入に加え、Retry 1の3問題を再現した壊したfixtureも、検査が失敗を検知することを確認した。
...
6. copy inventory 53状態を実画面と突き合わせ、未棚卸しの表示文言がないか独立に確認する。
