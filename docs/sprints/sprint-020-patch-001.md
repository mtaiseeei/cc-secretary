...
対象はChatworkの自然な日本語と詳細開閉の見た目、Google Chatの本人管理者向け画像ガイド、初回取り込みと自動取得設定を一度で確定する導線、理解テストの証跡化である。
...
- Google ChatはChatworkと同じ一体型フローにする。スペース→間隔→保存内容・共同編集者可視性・自動取得・commit／pushの確認→`この設定で始める` の1回で、初回取り込みと自動取得設定を完了する。手動のみでも初回取り込みは行い、scheduleは作らない。
- 自動間隔では、初回取り込み後に別の自動取得CTA、スペース／間隔の再選択、追加の設定変更フローを出さない。完了画面のprimary CTAは `設定を終了する` とする。
- 初回取り込みとschedule設定の結果が分かれる場合は、完了したこと、未完了のこと、次にすることを分けて示し、全体を成功と誤認させない。安全なtransaction／rollbackの既存境界は維持する。
...
- Chatwork／Google Chatの保存形式、取得間隔、3時間推奨・初期値、検索、再認証、後始末の振る舞い変更。ただし本scope changeで明記したGoogle Chat初回取り込みと自動取得設定の一体型確定、手動のみschedule 0件、部分失敗の結果表示は対象内とする。
...
10. **失敗と完了（C4/C8）**: 失敗は「何が起きたか→次にすること」、完了は「結果→次の一手」の上限内。英語エラー・診断・実行IDは主説明より後ろ。Google Chatは `この設定で始める` の1回で初回取り込みと自動取得設定を確定し、完了画面のprimaryは `設定を終了する`。自動取得の追加CTA、スペース／間隔の再選択、追加設定フロー0件。部分失敗時は完了・未完了・次の行動を分け、全体成功と誤表示しない。
...
14. **browser実操作（C3/C8）**: 開始→準備→接続→対象選択→間隔→確認→一体型確定→完了、0件、手動のみ、初回取り込み／scheduleの部分失敗、戻る、キャンセル、details、本人管理者ガイドをbrowserで確認する。静的grepだけで合格にしない。
...
- Google Chatでスペース→間隔→安全確認→`この設定で始める`→初回取り込み＋自動取得設定→`設定を終了する` を操作した記録。自動間隔の追加CTA／再選択／追加設定フロー0件、手動のみの初回取り込みあり・schedule 0件、初回取り込み／schedule部分失敗の正直な結果を含む。接続用ファイル入力の別回帰記録。
