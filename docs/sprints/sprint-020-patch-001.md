...
2026-07-18の最終評価中に行われたユーザー本人の手動UXレビューを、本Patch内のscope changeとして取り込む。
対象はChatworkの自然な日本語と詳細開閉の見た目、Google Chatの本人管理者向け画像ガイド、初回取得後の終了／任意の自動取得導線、理解テストの証跡化である。
既存のOAuth・同期・安全境界は変更せず、Retry Countの扱いはオーケストレーターの `state.md` に委ねる。
...
- 「詳しい説明」「管理者向け」は、山形アイコン等の明示的な開閉表示を持ち、閉じた状態と開いた状態が見た目で分かる。native detailsを維持してよいが、keyboard開閉、visible focus、内容を表すaccessible name、支援技術へ伝わるopen状態を満たす。
...
- Chatworkの外部画面で準備する場面は「用意できたら、この設定画面へ戻ってください」ではなく、「用意できたら、この設定画面へアクセスしてください」の意味を持つ自然な日本語にする。
- 主説明では目的を先に示し、`API Token` と `Repository Secret` の正式名称は取得画面や登録画面で照合が必要な箇所、または詳しい説明に残す。
- 取得対象は利用者が選んだルームだけであり、選択していないルームは読まない。

#### Google Chat

- 画面を開いている本人がGoogle Workspace管理者またはGoogle Cloudプロジェクトを作成できる場合を主経路とし、管理者へ依頼する経路も残す。
- 本人向けの折りたたみガイドでは、各社所有Google Cloudプロジェクトの作成、Google Chat API／People APIの有効化、OAuth同意画面の `Internal` 設定、`Desktop app` OAuth clientの作成、接続用ファイルの取得を、アカウント中立な画像つきで順に案内する。長い手順で主導線を重くしない。
- ガイド画像はメールアドレス、client ID、client secret、project固有の機密値・識別値を含まない。ガイド本文へ「Google側UIは2026年7月確認で、今後変わる可能性がある」旨を示す。
...
- Google Chatの初回取得結果では「設定を終了する」をprimary CTAにする。自動取得は任意のsecondary導線とし、「最初の取得は完了したが、自動取得はまだ有効ではない」意味を短く示す。自動取得を選んでも、直前に選択済みのスペースと間隔を再選択させず、確認→適用→終了へ進める。
...
- detailsのsummaryは開閉できることが見た目で分かる山形アイコン等を持ち、open時に向きまたは状態表示が変わる。mouseだけに依存せずkeyboard、visible focus、accessible name、open状態を検証する。
...
- 非エンジニア想定の初見理解テストは最低3回の独立セッションで構成する。2026-07-18のユーザー本人による手動レビューを独立した人間セッション1件としてEvaluatorが証跡化し、残り2件は実装担当ではない評価者による画面レビューとする。実参加者か独立Agentかを証跡に明記し、実参加者でない結果をhuman testと表現しない。
- ユーザー本人のセッションはChatwork／Google Chatとも5問すべて問題なし（5/5、重大誤解0）という確認済み事実として扱う。Evaluatorはユーザー発言、対象サービス、5問、採点、重大誤解0を記録し、同じセッションを推測で再採点しない。
- 残り2件の理解テストは秘密を含まないrunning UI、画面キャプチャ、DOM等を使った独立した画面レビューでよい。OSのファイル選択ダイアログを自動操作することを理解度評価の必須条件にしない。接続用ファイル入力の機能回帰はbrowser/CDP等で別に検証する。
...
- Chatwork／Google Chatの保存形式、取得間隔、3時間推奨・初期値、検索、再認証、後始末の振る舞い変更。ただし本scope changeで明記した「Google Chat初回取得後の終了をprimaryにし、任意の自動取得で再選択を要求しない」画面遷移は対象内とする。
...
4. **自然な日本語（C4/C7）**: 不自然な直訳、主語不足、英日混在、二重表現がinventoryレビューと実画面で0件。Chatworkの外部準備後は「この設定画面へアクセスしてください」の意味を使い、「戻ってください」を残さない。CTAだけを読んでも次の結果が分かる。
...
8. **Google Chat固有準備（C1/C4/C8/C11）**: 本人管理者を主経路、管理者依頼を副経路として選べる。本人向け折りたたみガイドで各社所有Cloud project、必要API、`Internal`、Desktop app、接続用ファイルの準備を画像つき・1画面1判断で案内する。画像はアカウント中立で機密・固有識別値0件、Google側UIの確認日と変更可能性を示す。読むだけ、`SPACE`限定、DM／グループDM除外、厳格secret非露出を維持する。
9. **0件・手動のみ・履歴保持（C3/C4/C5）**: 初回0件を失敗扱いせず、選択0件／手動のみでは今後の自動取得停止と取得済み履歴保持を自然な日本語で区別する。機能はsprint-020受入結果から変わらない。
10. **失敗と完了（C4/C8）**: 失敗は「何が起きたか→次にすること」、完了は「結果→次の一手」の上限内。英語エラー・診断・実行IDは主説明より後ろ。Google Chat初回結果では `設定を終了する` がprimary、自動取得が任意のsecondaryであり、自動取得を選んでもスペースと間隔を再選択せず確認→適用→終了する。
11. **両サービス整合（C2/C4/C8）**: 共通の画面役割では同じ情報順と用語原則を使い、サービス固有準備は混同しない。全画面でサービス名を可視・accessibleに明示する。
12. **CTA色・accessibility（C8）**: Chatwork `#F03747`、Google Chat `#11BB62`、前景 `#000000`、contrast 4.5:1以上。keyboard、focus、label、accessible name、error関連付け、読み上げ順が成立する。「詳しい説明」は山形アイコン等で開閉可能と分かり、closed／openの見た目、keyboard、visible focus、accessible name、open状態が成立する。
13. **desktop／mobile／200%（C4/C8）**: 両サービスの主要導線と代表状態をrunning UIで操作し、欠落・重複・横overflowなし。各幅と200%相当の秘密値を含まないスクリーンショットがある。
14. **browser実操作（C3/C8）**: 開始→準備→接続→対象選択→間隔→確認→完了、0件、手動のみ、失敗、戻る、キャンセル、details、本人管理者ガイド、Google Chat初回終了／任意の自動取得をbrowserで確認する。静的grepだけで合格にしない。
15. **初見理解テスト（C4/C7）**: 独立セッション3件で各サービス平均4/5以上、安全項目の重大な誤解0件。内訳は、ユーザー本人による確認済みの人間セッション1件（両サービス5/5、重大誤解0）と、実装担当ではない独立画面レビュー2件。評価主体、質問、回答、採点、使用画面、修正・再試験を記録する。OSファイル選択自動化は理解度評価の必須条件にしない。
16. **回帰の質（C2/C6）**: primary禁止語、必須意味、button／heading／label、detailsの開閉表示・状態、DOM構造、状態遷移をassertし、全文一致だけへ依存しない。接続用ファイル入力はbrowser/CDP等で実inputへ渡して別途検証し、壊したfixtureがそれぞれ失敗する。
...
- 初見理解テスト3件。2026-07-18のユーザー本人セッション1件（Chatwork／Google Chat各5/5、重大誤解0）を人間セッションとして記録し、残り2件の独立画面レビューについて評価主体、使用画面、質問、回答、採点、安全上の誤解、修正後の再試験を含む。
- Google Cloud本人設定ガイドの各画像、アカウント中立性・機密／固有識別値0件・確認日の検査、detailsのclosed／open・keyboard・focus・accessible name・open状態のbrowser証跡。
- Google Chat初回結果で終了がprimary、自動取得がsecondary、自動取得選択時にスペース／間隔の再選択0回で確認→適用→終了する操作記録。接続用ファイル入力の別回帰記録。
