...
> 事後注記（本パッチは合格・done・コミット済み。契約は合格時点の受入基準で確定）: Codex レビュー finding 7（`templates/AGENTS.md` の「この家」メタファー残存・旧「言い換え併記」規定の残存、およびメタファー検出の「家」系全般への拡張）は、本パッチの遡及拡張ではなく **`sprint-003-patch-001` で対応**する。
