...
- 5: Google Workspace組織所有Cloud projectと `Internal` Audience、Desktop appのPKCE＋state付きloopback、最小read-only scope、Repository Secret直接登録、厳格secret非露出、`SPACE`限定、DM／group DM／添付本文0件、選択対象だけの冪等保存、同意済み3時間推奨scheduleがすべて成立する。個人Googleアカウント、`External`、Test users、公開審査への利用者向け分岐0件。
