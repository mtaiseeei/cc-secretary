...
28. **Google Chat本人設定と完了分岐**: 本人管理者を主経路、管理者依頼を副経路として操作する。本人向け折りたたみガイドの画像がプロジェクト作成、必要API、`Internal`、Desktop app、接続用ファイル取得を覆い、メールアドレス、client ID、client secret、project固有の機密値・識別値0件、Google側UIの確認日ありと確認する。初回取得後は終了がprimary、自動取得がsecondaryで、自動取得を選んでもスペース／間隔を再選択せず確認→適用→終了する。
...
19. **Google Chat未接続**: 「Google Chatにつなぎたい」で高度な設定であることを示し、本人管理者の画像つき設定を主経路、管理者依頼を副経路として選べる。`Internal`／Desktop app／必要APIを順に示し、資格情報を会話へ貼るよう求めない。
...
- 5: desktop／mobile／200%相当の実画面で、1 step 1 message、1画面1判断、CTA最大2、画面別の最大情報量、technical detailの段階表示、余白中心の階層、指定palette・4px radius・14px中心が一貫し、keyboard／focus／label／contrastを含む主要操作が迷いなく完了する。detailsは開閉可能と分かる山形アイコン等、closed／open表示、keyboard、visible focus、accessible name、open状態を備える。Google Chatの本人設定ガイドと初回取得後の終了／任意の自動取得もループなく完了する。Tesla商標・写真・gradient・shadow・scale hoverは0件。
...
| 020-patch-001 | Chatwork／Google Chat全画面copy inventory、primary pathの難語除去、自然な「アクセス」表現、1画面1判断、開閉が分かるtechnical detail、Google Chat本人管理者向け画像ガイド、初回終了／任意自動取得の非ループ導線、0件／手動のみ／失敗／完了copy、desktop／mobile／200%、人間1＋独立画面レビュー2の理解テスト、実ファイル入力の別回帰、安全意味の欠落0件、全回帰 |
