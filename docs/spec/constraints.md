...
11. Google Chatの初回取得完了後は `設定を終了する` をprimary CTAとし、自動取得を任意のsecondary導線にする。自動取得を選んだ場合も、直前に選択したスペースと間隔を再選択させず、確認→適用→終了へ進める。初回取得と自動取得が別であることを短く示し、同じ設定を繰り返すループに見せない。
