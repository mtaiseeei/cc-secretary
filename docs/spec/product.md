...
中心思想は **「1つのprivate GitHub repoに、秘書・開発・Chatworkの文脈をまとめる」**。
秘書の記憶と成果物、通常のプロジェクト開発、選択したChatwork roomの履歴を同じrepoでGit管理し、
秘書と開発ハーネスが同じ作業文脈を読めるようにする。Chatwork以外の外部データは、従来どおり
公式コネクタで都度参照する。

publicな `yasashii-secretary` repoは配布物の正本であり、利用者データの保存先ではない。
各利用者はpluginを使う秘書ワークスペース、通常のproject開発、Chatwork設定・workflow・履歴を
1つのprivate GitHub repoに置く。実API評価用repoもこの実利用構成を再現する専用private test workspaceとし、
Chatwork専用repoへ分離しない。
...
このrepoが、プラグインを使う秘書ワークスペース、通常のプロジェクト開発、秘書の記憶と成果物、
Chatwork履歴の唯一の作業単位になる。Chatwork専用repoや、Chatwork以外だけを別のローカル領域へ分けない。
