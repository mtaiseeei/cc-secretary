...
| Chatwork接続 | Repository Secretにあるtokenを使う読取専用接続 | GitHub Actions |
| room選択 | ユーザーが保存対象として明示したRoom ID集合 | 同じrepoのChatwork設定 |
| Chatwork履歴 | 選択roomから取得済みのメッセージ | 同じrepoのChatwork履歴領域 |
| 同期状態 | 最終成功、roomごとの取得位置、失敗理由 | 同じrepoの状態記録 |
...
| 選択 | 30日換算の概算run数 | 実行の意味 |
|---|---:|---|
| 30分 | 1440 | 毎時17分・47分を起点 |
| 1時間 | 720 | 毎時17分を起点。既定推奨 |
| 3時間 | 240 | 3時間ごとの17分を起点 |
| 6時間 | 120 | 6時間ごとの17分を起点 |
| 12時間 | 60 | 12時間ごとの17分を起点 |
| 手動のみ | 0 | scheduleなし |

run数は実行回数の概算であり、GitHub Actionsの実課金分数ではない。
