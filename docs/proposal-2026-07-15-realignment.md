...
- NG/OK 例ペア方式を採用（例示は抽象指示より効く）。ただし濃いキャラクター（関西弁・執事風等）は初期同梱しない — 検証不能面が広がるため、塾コンテンツ「例ペアを自分で育てるチュートリアル」として出す方が訴求も検証コストも良い（実現性批評8。要確認 → 8章）
...
| 4 | 濃いキャラプリセット（関西弁・執事風等）の同梱 | 同梱しない。塾コンテンツ「例ペアを育てるチュートリアル」として出す |
| 5 | dashboard.html | timeline 先行（G1 達成の最小可視化＝timeline と定義）。dashboard は反応を見て sprint-012 で判断 |
| 6 | 既存塾生向けの移行シーム（journal/ 新設・preferences v1→v2 の migrate） | 第2期配布は新規セットアップ想定なら不要。既存ユーザーがいるなら sprint-012 に追加 |
