...
19. 詳細は開閉可能と見た目・支援技術の両方で分かるようにする。Google ChatのCloud準備は本人管理者を主経路、管理者依頼を副経路とし、アカウント中立な画像で案内する。スペース・間隔・保存内容を1回確認して初回取り込みと自動取得設定を同じ確定操作で完了し、完了画面は終了だけをprimaryにする。
